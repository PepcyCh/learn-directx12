...
cbuffer obj_cb : register(b0) {
    ...
    float2 displacement_map_texel_size;
    float grid_spatial_step;
    float obj_cb_padding0;
};
...
VertexOut VS(VertexIn vin) {
    ...
    vin.pos.y += displacement_map.SampleLevel(sam_point_wrap, vin.texc, 0.0f);

    float du = displacement_map_texel_size.x;
    float dv = displacement_map_texel_size.y;
    float l = displacement_map.SampleLevel(sam_point_wrap, vin.texc - float2(du, 0.0f), 0.0f);
    float r = displacement_map.SampleLevel(sam_point_wrap, vin.texc + float2(du, 0.0f), 0.0f);
    float t = displacement_map.SampleLevel(sam_point_wrap, vin.texc - float2(0.0f, dv), 0.0f);
    float b = displacement_map.SampleLevel(sam_point_wrap, vin.texc + float2(0.0f, dv), 0.0f);
    vin.norm = normalize(float3(l - r, 2.0f * grid_spatial_step, b - t));
#endif
    VertexOut vout;
