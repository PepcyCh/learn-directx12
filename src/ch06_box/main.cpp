...
// OpenGL:
// * column-major matrix
// * left-multiply column vector
// * ccw front face by default
//
// DirectX:
// * row-major matrix
// * right-multiply row vector
// * cw front face by default
//
// use RH matrices and set ccw front face, then DirectX will produce the same result as what OpenGL does

class D3DAppBox : public D3DApp {
  public:
    D3DAppBox(HINSTANCE h_inst) : D3DApp(h_inst) {}
    ~D3DAppBox() {}
  ...
  private:
    ...
    void Update(const Timer &timer) override {
        ...
        XMMATRIX _view = XMMatrixLookAtLH(pos, lookat, up); // LH is for left-hand
    ...
    void BuildBoxGeometry() {
        ...
        p_mesh = std::make_unique<MeshGeometry>();
        p_mesh->name = "box_mesh";

        // create blob and copy data into it for vb and ib
        // D3DCreateBlob is in <d3dcompiler.h> ...
        ThrowIfFailed(D3DCreateBlob(vb_size, &p_mesh->vb_cpu));
        CopyMemory(p_mesh->vb_cpu->GetBufferPointer(), vertices.data(), vb_size);
        ThrowIfFailed(D3DCreateBlob(ib_size, &p_mesh->ib_cpu));
        CopyMemory(p_mesh->ib_cpu->GetBufferPointer(), indices.data(), ib_size);

        // create buffer that holds the data for vb and ib
        p_mesh->vb_gpu = D3DUtil::CreateDefaultBuffer(p_device.Get(), p_cmd_list.Get(),
            vertices.data(), vb_size, p_mesh->vb_uploader);
        p_mesh->ib_gpu = D3DUtil::CreateDefaultBuffer(p_device.Get(), p_cmd_list.Get(),
            indices.data(), ib_size, p_mesh->ib_uploader);

        // other data of vb and ib
        p_mesh->vb_stride = sizeof(Vertex);
        p_mesh->vb_size = vb_size;
    ...
    std::unique_ptr<MeshGeometry> p_mesh = nullptr;
