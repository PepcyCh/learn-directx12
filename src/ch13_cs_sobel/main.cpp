...
// use Sobel Filter to ditect silhouette (?) and draw the silhouette with a 'composite' step
// use 'cartoon styled lighting' mentioned in exercise 6 in Chapter 8

class D3DAppSobel : public D3DApp {
  public:
    D3DAppSobel(HINSTANCE h_inst) : D3DApp(h_inst) {}
    ~D3DAppSobel() {
        if (p_device != nullptr) {
            FlushCommandQueue();
        }
    ...
    bool Initialize() override {
        ...
        p_render_target = std::make_unique<RenderTarget>(p_device.Get(), client_width, client_height,
            back_buffer_fmt);
        p_sobel_filter = std::make_unique<SobelFilter>(p_device.Get(), client_width, client_height,
            back_buffer_fmt);
  ...
  private:
    void CreateDescriptorHeaps() override {
        // rtv (1 more rtv for RenderTarget)
        D3D12_DESCRIPTOR_HEAP_DESC rtv_heap_desc;
        rtv_heap_desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;
        rtv_heap_desc.NumDescriptors = kSwapChainBufferCnt + 1;
    ...
    void OnResize() override {
        ...
        if (p_render_target) {
            p_render_target->OnResize(client_width, client_height);
        }
        if (p_sobel_filter) {
            p_sobel_filter->OnResize(client_width, client_height);
        }
    }
    ...
    void Draw(const Timer &timer) override {
        ...
        // render target: read -> render target
        D3D12_RESOURCE_BARRIER rt_read2rt = CD3DX12_RESOURCE_BARRIER::Transition(p_render_target->Resource(),
            D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_RENDER_TARGET);
        p_cmd_list->ResourceBarrier(1, &rt_read2rt);

        // clear rtv and dsv
        auto render_target_view = p_render_target->Rtv();
        auto depth_stencil_view = DepthStencilView();
        p_cmd_list->ClearRenderTargetView(render_target_view, (float *) &main_pass_cb.fog_color, 0, nullptr);
        p_cmd_list->ClearDepthStencilView(depth_stencil_view, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL,
            1.0f, 0, 0, nullptr);
        // set rtv and dsv
        p_cmd_list->OMSetRenderTargets(1, &render_target_view, true, &depth_stencil_view);
        ...
        // post process; render target : render target -> read
        D3D12_RESOURCE_BARRIER rt_rt2read = CD3DX12_RESOURCE_BARRIER::Transition(p_render_target->Resource(),
            D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_GENERIC_READ);
        p_cmd_list->ResourceBarrier(1, &rt_rt2read);
        p_sobel_filter->Execute(p_cmd_list.Get(), p_post_rt_sig.Get(), psos["sobel"].Get(),
            p_render_target->Srv());

        // composite; back buffer: present -> render target
        D3D12_RESOURCE_BARRIER back_present2rt = CD3DX12_RESOURCE_BARRIER::Transition(CurrBackBuffer(),
		    D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
        p_cmd_list->ResourceBarrier(1, &back_present2rt);

        auto back_buffer_view = CurrBackBufferView();
        p_cmd_list->OMSetRenderTargets(1, &back_buffer_view, true, &depth_stencil_view);

        p_cmd_list->SetGraphicsRootSignature(p_post_rt_sig.Get());
        p_cmd_list->SetPipelineState(psos["composite"].Get());
        p_cmd_list->SetGraphicsRootDescriptorTable(0, p_render_target->Srv());
        p_cmd_list->SetGraphicsRootDescriptorTable(1, p_sobel_filter->Srv());
        DrawScreenQuad(p_cmd_list.Get());

        // back buffer: render target -> present
        D3D12_RESOURCE_BARRIER back_rt2present = CD3DX12_RESOURCE_BARRIER::Transition(CurrBackBuffer(),
		    D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
        p_cmd_list->ResourceBarrier(1, &back_rt2present);
    ...
    void LoadTextures() {
        ...
        auto whilte_tex = std::make_unique<Texture>();
        whilte_tex->name = "white";
        whilte_tex->filename = root_path + L"textures/white1x1.dds";
        ThrowIfFailed(CreateDDSTextureFromFile(p_device.Get(), resource_upload,
            whilte_tex->filename.c_str(), &whilte_tex->resource));
        textures[whilte_tex->name] = std::move(whilte_tex);
    ...
    void BuildPostRootSignature() {
        ...
        CD3DX12_DESCRIPTOR_RANGE srv_range0(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0);
        rt_params[0].InitAsDescriptorTable(1, &srv_range0);
        CD3DX12_DESCRIPTOR_RANGE srv_range1(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1);
        rt_params[1].InitAsDescriptorTable(1, &srv_range1);
    ...
    void BuildDescriptorHeaps() {
        ...
        srv_heap_desc.NumDescriptors = textures.size() + p_wave->DescriptorCount() +
            p_render_target->DescriptorCount() + p_sobel_filter->DescriptorCount();
        ...
        offset += textures.size();

        // wave
        p_wave->BuildDescriptors(
            CD3DX12_CPU_DESCRIPTOR_HANDLE(p_cbv_srv_uav_heap->GetCPUDescriptorHandleForHeapStart(),
                offset, cbv_srv_uav_descriptor_size),
            CD3DX12_GPU_DESCRIPTOR_HANDLE(p_cbv_srv_uav_heap->GetGPUDescriptorHandleForHeapStart(),
                offset, cbv_srv_uav_descriptor_size),
            cbv_srv_uav_descriptor_size);
        offset += p_wave->DescriptorCount();

        // render target
        p_render_target->BuildDescriptors(
            CD3DX12_CPU_DESCRIPTOR_HANDLE(p_cbv_srv_uav_heap->GetCPUDescriptorHandleForHeapStart(),
                offset, cbv_srv_uav_descriptor_size),
            CD3DX12_GPU_DESCRIPTOR_HANDLE(p_cbv_srv_uav_heap->GetGPUDescriptorHandleForHeapStart(),
                offset, cbv_srv_uav_descriptor_size),
            CD3DX12_CPU_DESCRIPTOR_HANDLE(p_rtv_heap->GetCPUDescriptorHandleForHeapStart(),
                kSwapChainBufferCnt, rtv_descriptor_size));
        offset += p_render_target->DescriptorCount();

        // sobel filter
        p_sobel_filter->BuildDescriptors(
            CD3DX12_CPU_DESCRIPTOR_HANDLE(p_cbv_srv_uav_heap->GetCPUDescriptorHandleForHeapStart(),
                offset, cbv_srv_uav_descriptor_size),
            CD3DX12_GPU_DESCRIPTOR_HANDLE(p_cbv_srv_uav_heap->GetGPUDescriptorHandleForHeapStart(),
                offset, cbv_srv_uav_descriptor_size),
    ...
    void BuildShaderAndInputLayout() {
        ...
        shaders["standard_vs"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/P3N3U2_default.hlsl",
            nullptr, "VS", "vs_5_1");
        shaders["displacement_vs"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/P3N3U2_default.hlsl",
            displacement_defines, "VS", "vs_5_1");
        shaders["opaque_ps"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/P3N3U2_default.hlsl",
            fog_defines, "PS", "ps_5_1");
        shaders["alpha_tested_ps"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/P3N3U2_default.hlsl",
            alpha_test_defines, "PS", "ps_5_1");
        shaders["wave_update_cs"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/wave.hlsl",
            nullptr, "UpdateWaveCS", "cs_5_1");
        shaders["wave_disturb_cs"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/wave.hlsl",
            nullptr, "DisturbCS", "cs_5_1");
        shaders["sobel_cs"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/sobel.hlsl",
            nullptr, "SobelCS", "cs_5_1");
        shaders["composite_vs"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/composite.hlsl",
            nullptr, "VS", "vs_5_1");
        shaders["composite_ps"] = D3DUtil::CompileShader(src_path + L"ch13_cs_sobel/shaders/composite.hlsl",
            nullptr, "PS", "ps_5_1");
    ...
    void BuildMaterials() {
        ...
        grass->albedo = { 0.25f, 1.0f, 0.25f, 1.0f };
        ...
        water->mat_cb_ind = 1;
        water->diffuse_srv_heap_index = 0;
        water->albedo = { 0.15f, 0.15f, 1.0f, 0.8f }; // transparent water
        ...
        fence->mat_cb_ind = 2;
        fence->diffuse_srv_heap_index = 0;
        fence->albedo = { 0.7f, 0.7f, 0.7f, 1.0f };
    ...
    void BuildPSOs() {
        ...
        D3D12_GRAPHICS_PIPELINE_STATE_DESC composite_pso_desc = opaque_pso_desc;
        composite_pso_desc.pRootSignature = p_post_rt_sig.Get();
        composite_pso_desc.VS = {
            reinterpret_cast<BYTE *>(shaders["composite_vs"]->GetBufferPointer()),
            shaders["composite_vs"]->GetBufferSize()
        };
        composite_pso_desc.PS = {
            reinterpret_cast<BYTE *>(shaders["composite_ps"]->GetBufferPointer()),
            shaders["composite_ps"]->GetBufferSize()
        };
        composite_pso_desc.DepthStencilState.DepthEnable = false;
        ThrowIfFailed(p_device->CreateGraphicsPipelineState(&composite_pso_desc,
            IID_PPV_ARGS(&psos["composite"])));
        ...
        D3D12_COMPUTE_PIPELINE_STATE_DESC sobel_pso_desc = {};
        sobel_pso_desc.pRootSignature = p_post_rt_sig.Get();
        sobel_pso_desc.CS = {
            reinterpret_cast<BYTE *>(shaders["sobel_cs"]->GetBufferPointer()),
            shaders["sobel_cs"]->GetBufferSize()
        };
        sobel_pso_desc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
        ThrowIfFailed(p_device->CreateComputePipelineState(&sobel_pso_desc,
            IID_PPV_ARGS(&psos["sobel"])));
    }
    ...
    std::unique_ptr<RenderTarget> p_render_target;
    std::unique_ptr<SobelFilter> p_sobel_filter;
